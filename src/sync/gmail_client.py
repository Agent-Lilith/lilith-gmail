...
class GmailClient:
    ...
    def setup_push_notifications(
        ...
        return self.service.users().watch(userId=self.user_id, body=request).execute()
    ...
    def parse_body(payload: dict[str, Any]) -> str:
        if "parts" in payload:
            ...
            if text_part and text_part["body"].get("data"):
                return base64.urlsafe_b64decode(text_part["body"]["data"]).decode(
                    "utf-8"
                )
            if html_part and html_part["body"].get("data"):
                html = base64.urlsafe_b64decode(html_part["body"]["data"]).decode(
                    "utf-8"
                )
